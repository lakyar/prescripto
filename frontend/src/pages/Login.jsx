  ...
  return (
    <form className='min-h-[80vh] flex items-center' action="">
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-md'>
        ...
        <button className='bg-primary text-white w-full py-2 rounded-md mt-3'>{state === 'Sign Up' ? "Create Account" : "Login"}</button>
        {
          state === "Sign Up"
          ? <p>Already have an account? <span onClick={()=>setState('Login')} className='text-primary select-none underline cursor-pointer'>Login here!</span></p>
          : <p>Create new account? <span onClick={()=>setState('Sign Up')} className='text-primary select-none underline cursor-pointer'>Click here!</span></p>
        }
      </div>
    </form>
  )
